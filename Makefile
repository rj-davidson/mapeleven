...
docker-up-controller-detached: docker-build-api-db
	@echo "+ $@"
	docker-compose --env-file $(ENV_FILE) up -d api-db

docker-up-server-detached: docker-build-api-server
	@echo "+ $@"
	docker-compose --env-file $(ENV_FILE) up -d api-server
