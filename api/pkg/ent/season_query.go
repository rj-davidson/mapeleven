...
func (sq *SeasonQuery) QueryPlayerStats() *PlayerStatsQuery {
	query := (&PlayerStatsClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SeasonQuery) Clone() *SeasonQuery {
	...
	return &SeasonQuery{
		...
		withPlayerStats: sq.withPlayerStats.Clone(),
...
func (sq *SeasonQuery) WithPlayerStats(opts ...func(*PlayerStatsQuery)) *SeasonQuery {
	query := (&PlayerStatsClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withPlayerStats = query
	return sq
}
...
func (sq *SeasonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Season, error) {
	...
	if query := sq.withPlayerStats; query != nil {
		if err := sq.loadPlayerStats(ctx, query, nodes,
			func(n *Season) { n.Edges.PlayerStats = []*PlayerStats{} },
			func(n *Season, e *PlayerStats) { n.Edges.PlayerStats = append(n.Edges.PlayerStats, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SeasonQuery) loadPlayerStats(ctx context.Context, query *PlayerStatsQuery, nodes []*Season, init func(*Season), assign func(*Season, *PlayerStats)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Season)
	nids := make(map[int]map[*Season]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(season.PlayerStatsTable)
		s.Join(joinT).On(s.C(playerstats.FieldID), joinT.C(season.PlayerStatsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(season.PlayerStatsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(season.PlayerStatsPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Season]struct{}{byID[outValue]: {}}
