...
func (fq *FixtureQuery) Order(o ...fixture.OrderOption) *FixtureQuery {
...
func (fq *FixtureQuery) QueryLineups() *FixtureLineupsQuery {
	query := (&FixtureLineupsClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (fq *FixtureQuery) Clone() *FixtureQuery {
	...
	return &FixtureQuery{
		config:            fq.config,
		ctx:               fq.ctx.Clone(),
		order:             append([]fixture.OrderOption{}, fq.order...),
...
func (fq *FixtureQuery) WithLineups(opts ...func(*FixtureLineupsQuery)) *FixtureQuery {
	query := (&FixtureLineupsClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FixtureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Fixture, error) {
	...
	if query := fq.withLineups; query != nil {
...
func (fq *FixtureQuery) loadLineups(ctx context.Context, query *FixtureLineupsQuery, nodes []*Fixture, init func(*Fixture), assign func(*Fixture, *FixtureLineups)) error {
	...
	query.Where(predicate.FixtureLineups(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(fixture.LineupsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.fixture_lineups
		if fk == nil {
			return fmt.Errorf(`foreign-key "fixture_lineups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "fixture_lineups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (fq *FixtureQuery) loadFixtureEvents(ctx context.Context, query *FixtureEventsQuery, nodes []*Fixture, init func(*Fixture), assign func(*Fixture, *FixtureEvents)) error {
	...
	query.Where(predicate.FixtureEvents(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(fixture.FixtureEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "fixture_fixture_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
