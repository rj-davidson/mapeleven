...
type FixtureLineupsQuery struct {
	config
	ctx              *QueryContext
	order            []fixturelineups.OrderOption
	inters           []Interceptor
...
func (flq *FixtureLineupsQuery) Order(o ...fixturelineups.OrderOption) *FixtureLineupsQuery {
...
func (flq *FixtureLineupsQuery) Clone() *FixtureLineupsQuery {
	...
	return &FixtureLineupsQuery{
		config:           flq.config,
		ctx:              flq.ctx.Clone(),
		order:            append([]fixturelineups.OrderOption{}, flq.order...),
...
func (flq *FixtureLineupsQuery) loadLineupPlayer(ctx context.Context, query *MatchPlayerQuery, nodes []*FixtureLineups, init func(*FixtureLineups), assign func(*FixtureLineups, *MatchPlayer)) error {
	...
	query.Where(predicate.MatchPlayer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(fixturelineups.LineupPlayerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "fixture_lineups_lineup_player" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
