...
func (cq *CountryQuery) loadPlayers(ctx context.Context, query *PlayerQuery, nodes []*Country, init func(*Country), assign func(*Country, *Player)) error {
	...
	query.Where(predicate.Player(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.PlayersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "country_players" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CountryQuery) loadLeagues(ctx context.Context, query *LeagueQuery, nodes []*Country, init func(*Country), assign func(*Country, *League)) error {
	...
	query.Where(predicate.League(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.LeaguesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "country_leagues" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CountryQuery) loadClubs(ctx context.Context, query *ClubQuery, nodes []*Country, init func(*Country), assign func(*Country, *Club)) error {
	...
	query.Where(predicate.Club(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.ClubsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "country_clubs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
