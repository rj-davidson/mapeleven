...
func (sq *SquadQuery) QueryPlayer() *PlayerQuery {
...
func (sq *SquadQuery) QueryTeam() *TeamQuery {
...
func (sq *SquadQuery) QuerySeason() *SeasonQuery {
...
func (sq *SquadQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (sq *SquadQuery) FirstIDX(ctx context.Context) int {
...
func (sq *SquadQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (sq *SquadQuery) OnlyIDX(ctx context.Context) int {
...
func (sq *SquadQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (sq *SquadQuery) IDsX(ctx context.Context) []int {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *SquadQuery) WithPlayer(opts ...func(*PlayerQuery)) *SquadQuery {
	query := (&PlayerClient{config: sq.config}).Query()
...
func (sq *SquadQuery) WithTeam(opts ...func(*TeamQuery)) *SquadQuery {
	query := (&TeamClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SquadQuery) WithSeason(opts ...func(*SeasonQuery)) *SquadQuery {
	query := (&SeasonClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SquadQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Squad, error) {
	...
	if sq.withPlayer != nil || sq.withTeam != nil || sq.withSeason != nil {
		withFKs = true
	}
	if withFKs {
...
func (sq *SquadQuery) loadPlayer(ctx context.Context, query *PlayerQuery, nodes []*Squad, init func(*Squad), assign func(*Squad, *Player)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Squad)
	for i := range nodes {
		if nodes[i].player_squad == nil {
			continue
		}
		fk := *nodes[i].player_squad
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "player_squad" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SquadQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Squad, init func(*Squad), assign func(*Squad, *Team)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Squad)
	for i := range nodes {
		if nodes[i].team_squad == nil {
			continue
		}
		fk := *nodes[i].team_squad
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_squad" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SquadQuery) loadSeason(ctx context.Context, query *SeasonQuery, nodes []*Squad, init func(*Squad), assign func(*Squad, *Season)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Squad)
	for i := range nodes {
		if nodes[i].season_squad == nil {
			continue
		}
		fk := *nodes[i].season_squad
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "season_squad" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SquadQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(squad.Table, squad.Columns, sqlgraph.NewFieldSpec(squad.FieldID, field.TypeInt))
	_spec.From = sq.sql
